...
//                     const sql3 = `SELECT hs.id AS hostel_id, hs.Name AS hostel_name, sd.plan_code, sd.plan_start, sd.plan_end, sd.status AS plan_status 
//                                   FROM hosteldetails hs 
//                                   LEFT JOIN (
//                                       SELECT * FROM subscription_details WHERE status = 1 AND user_id = ?
...
//             } else {
//                 // Staff: get role permissions first
...
//                     // Step 1: Get admin_id who created this staff
//                     const admin_sql = `SELECT created_by FROM createaccount WHERE id = ? LIMIT 1`;
//                     connection.query(admin_sql, [created_by], function (adminErr, adminRes) {
...
//                         // Step 2: Get plan details from admin_id
//                         const plan_sql = `SELECT * FROM subscription_details WHERE user_id = ? ORDER BY id DESC LIMIT 1`;
//                         connection.query(plan_sql, [admin_id], function (err, plan_data) {
//                             if (err || !plan_data.length) {
//                                 return response.status(200).json({
//                                     message: "User Details",
//                                     user_details: filtered_user,
//                                     is_owner: 0,
...
//                             // Step 3: Get hostel details from admin_id
//                             const hostel_sql = `SELECT hs.id AS hostel_id, hs.Name AS hostel_name, sd.plan_code, sd.plan_start, sd.plan_end, sd.status AS plan_status 
//                                                 FROM hosteldetails hs 
//                                                 LEFT JOIN (
//                                                     SELECT * FROM subscription_details WHERE status = 1 AND user_id = ?
//                                                 ) sd ON hs.id = sd.hostel_id 
...
//                             connection.query(hostel_sql, [admin_id, admin_id], function (err, hostel_data) {
//                                 if (err) {
//                                     console.log(err);
//                                     return response.status(201).json({ message: "Error fetching hostel info", statusCode: 201 });
//                                 }
...
//             }
//         } else {
//             return response.status(201).json({ message: "Invalid User Details" });
//         }
//     });
// }
