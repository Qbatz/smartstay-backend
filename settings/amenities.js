            ...
            if (ch_default == 1) {
                ...
                connection.query(sql2, [am_id, hostel_id], function (err, sel_res) {
                    if (err) {
                        ...
                        var sql3 = `SELECT u.id, u.name AS user_Name, u.User_Id AS user_id FROM hostel u WHERE u.isActive = 1 AND u.Hostel_Id = ? AND NOT EXISTS (SELECT 1 FROM AmenitiesHistory ah WHERE ah.user_Id = u.user_Id AND ah.amenity_Id = ? AND ah.status = 1 AND ah.created_At = (SELECT MAX(created_At) FROM AmenitiesHistory WHERE user_Id = ah.user_Id AND amenity_Id = ah.amenity_Id))`;
                        connection.query(sql3, [hostel_id, am_id], function (err, unsel_res) {
                            if (err) {
                                console.log(err);
