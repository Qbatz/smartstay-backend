...
function checkout_list(req, res) {
  ...
  if (is_admin == 1 || (role_permissions[6] && role_permissions[6].per_view == 1)) {
    ...
    var sql1 = "SELECT hs.Hostel_Id,hs.ID,hs.HostelName,hs.Name,hs.checkout_comment,DATE_FORMAT(hs.CheckoutDate, '%Y-%m-%d') AS CheckoutDate,hs.profile AS user_profile,DATEDIFF(checkoutDate, '" + current_date + "') AS notice_period,hos_de.profile,hs.Floor,hs.Bed,hs.req_date,hs.isActive,hs.Phone,hf.floor_name AS floor_name,hosroom.Room_Id AS room_name,bed.bed_no AS bed_name FROM hostel AS hs JOIN hosteldetails AS hos_de ON hos_de.id=hs.Hostel_Id LEFT JOIN Hostel_Floor AS hf ON hf.floor_id=hs.Floor AND hf.hostel_id=hs.Hostel_Id LEFT JOIN hostelrooms AS hosroom ON hosroom.id=hs.Rooms LEFT JOIN bed_details AS bed ON bed.id=hs.Bed WHERE hs.Hostel_Id =? AND hs.CheckoutDate !='null' ORDER BY hs.ID DESC;";
