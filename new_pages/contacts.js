...
exports.all_announce = (req, res) => {
    ...
    var sql = "SELECT a.*, (SELECT COUNT(*) FROM announcement_likes al WHERE al.an_id = a.id AND al.status = 1) AS like_count,(SELECT COUNT(*) FROM announcement_comments al WHERE al.an_id = a.id AND a.status = 1) AS comment_count,ca.profile FROM announcements a LEFT JOIN createaccount AS ca ON ca.id=a.created_by WHERE a.status = 1 AND a.hostel_id = ?";
