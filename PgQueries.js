  ...
  if (
    ...
    connection.query(queryHostelList, function (err, hostels) {
      if (err) {
        ...
        return response
          .status(201)
          .json({ statusCode: 201, message: "Internal Server Error" });
      }

      if (!hostels || hostels.length === 0) {
        return response
          .status(201)
          .json({ statusCode: 201, message: "No Data Found" });
      }
        ...
        connection.query(queryFloorDetails, function (floorErr, floorDetails) {
          if (floorErr) {
            ...
            connection.query(
              ...
              function (additionalErr, additionalDetails) {
                if (additionalErr) {
                  console.error(
                  ...
                  return response.status(201).json({
                    statusCode: 201,
                    message: "Error fetching additional details",
                  });
                } else {
...
async function createPG(reqHostel, res, req) {
  ...
  if (!hostel_name || !hostel_phone || !pin_code || !city || !state) {
    return res
      .status(201)
      .json({ message: "Please Add All Required Fields", statusCode: 201 });
  }
  ...
  if (!profile) {
    ...
    profile_url = await uploadImage.uploadProfilePictureToS3Bucket(
      bucket_name,
      folderName,
      `${hostel_name}` + `${timestamp}` + `${profile.originalname}`,
      profile
    );
  ...
  if (!image1) {
    ...
    image1_url = await uploadImage.uploadProfilePictureToS3Bucket(
      bucket_name,
      folderName,
      `${hostel_name}` + `${timestamp}` + `${image1.originalname}`,
      image1
    );
  }
  ...
  if (!image2) {
    ...
    image2_url = await uploadImage.uploadProfilePictureToS3Bucket(
      bucket_name,
      folderName,
      `${hostel_name}` + `${timestamp}` + `${image2.originalname}`,
      image2
    );
  }
  ...
  if (!image3) {
    ...
    image3_url = await uploadImage.uploadProfilePictureToS3Bucket(
      bucket_name,
      folderName,
      `${hostel_name}` + `${timestamp}` + `${image3.originalname}`,
      image3
    );
  }
  ...
  if (!image4) {
    ...
    image4_url = await uploadImage.uploadProfilePictureToS3Bucket(
      bucket_name,
      folderName,
      `${hostel_name}` + `${timestamp}` + `${image4.originalname}`,
      image4
    );
  }
  ...
  if (id) {
    if (
      is_admin == 1 ||
      (role_permissions[3] && role_permissions[3].per_edit == 1)
    ) {
      ...
      connection.query(sql1, [id, created_by], function (err, hos_data) {
        if (err) {
          return res
            .status(201)
            .json({ message: "Unable to Get Hostel Details", statusCode: 201 });
        } else if (hos_data.length != 0) {
          var sql2 =
            "UPDATE hosteldetails SET Name=?,email_id=?,Address=?,area=?,landmark=?,pin_code=?,city=?,state=?,hostel_PhoneNo=?,profile=?,image1=?,image2=?,image3=?,image4=? WHERE id=?";
          connection.query(
            sql2,
            [
              hostel_name,
            ...
            function (err, up_data) {
              if (err) {
                ...
                var sql3 =
                  ...
                  " AND isActive=1;";
                var sql4 =
                  "UPDATE compliance SET hostelname='" +
                  hostel_name +
                  "' WHERE Hostel_id=" +
                  id +
                  " AND isActive=1;";
                var sql6 =
                  "UPDATE invoicedetails SET Hostel_Name='" +
                  hostel_name +
                  "' WHERE Hostel_Id=" +
                  id +
                  "";
    ...
    if (
      is_admin == 1 ||
      (role_permissions[3] && role_permissions[3].per_create == 1)
    ) {
      // var hostel_count = req.user_details.hostel_count;

      const sqlCount =
        "SELECT COUNT(id) AS hos_count FROM hosteldetails WHERE created_By=? AND isActive=1";
      connection.query(sqlCount, [created_by], function (error, result) {
        if (error) {
          console.log("Error fetching hostel count:", error);
          return res
            .status(201)
            .json({ statusCode: 201, message: "Error fetching hostel count" });
        } else if (result.length == 0) {
          const sql1 =
            "INSERT INTO hosteldetails(Name,hostel_PhoneNo,email_id,Address,area,landmark,pin_code,city,state,created_By,isHostelBased,profile,image1,image2,image3,image4) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
          connection.query(
            sql1,
            [
              hostel_name,
            ...
            function (error, data) {
              if (error) {
                ...
                return res
                  .status(201)
                  .json({ statusCode: 201, message: "Cannot Insert Details" });
              } else {
                ...
                template_type.forEach((type) => {
                  ...
                  connection.query(insertsql, values, (err, result) => {
                    if (err) {
                     ...
                     let prefix = type == 'Rental Invoice' ? 'INV' :'ADV';
                     let suffix = type == '001';

                    if (type == "Rental Invoice" || "Security Deposit Invoice") {
                      var updateQuery = ` UPDATE bill_template
                                            SET
                                                prefix = ?,
                                                suffix = ?
                      ...
                      connection.query(
                        updateQuery,
                        [prefix,suffix,data.insertId,result.insertId],
                        (err, result) => {
                             if (err) {
                      console.error("Insert failed for:", type, err.message);
                      return;
                    ...
                    if (completedInsert === template_type.length) {
                      console.log("Bill Template Inserted");
                    }
                  });
                });
                return res
                  .status(200)
                  .json({ statusCode: 200, message: "PG Added Succsessfully" });
              }
            }
          );
        } else {
...
`;
                ...
                template_type.forEach((type) => {
                  ...
                  connection.query(insertsql, values, (err, result) => {
                    ...
                    if (type === "Rental Invoice" || type === "Security Deposit Invoice") {
                        console.log("ty-----------------",prefix,suffix,type)
                      var updateQuery = ` UPDATE bill_template
                                            SET
                                                prefix = ?,
                                                suffix = ?
...
function RoomList(connection, request, response) {
  ...
  if (
    ...
    if (reqData) {
      connection.query(
        `select * from hostelrooms where Hostel_Id = \'${reqData.hostel_Id}\' and Floor_Id =  \'${reqData.floor_Id}\' and isActive=1`,
        function (error, data) {
          if (data) {
            response.status(200).json({ data: data });
          } else {
...
function RoomCount(connection, request, response) {
  ...
  if (
    ...
    if (reqFloorID) {
      ...
      connection.query(
        sql1,
        [reqFloorID.floor_Id, reqFloorID.hostel_Id],
        function (error, RoomsData) {
          if (error) {
            console.log(error);
            return response
              .status(201)
          ...
          if (RoomsData.length > 0) {
            let responseData = new Array(RoomsData.length); // Ensure order is maintained

            let promises = RoomsData.map((room, i) => {
              return new Promise((resolve, reject) => {
                const Room_Id = room.room_id;

                // Query to get booked bed count
                const bookedBedCountQuery = `SELECT COUNT('Bed') AS bookedBedCount FROM hostel 
                                             WHERE Floor = ? AND Hostel_Id = ? AND Rooms = ? AND isActive = true`;

                connection.query(
                  bookedBedCountQuery,
                  [reqFloorID.floor_Id, reqFloorID.hostel_Id, Room_Id],
                  function (error, hostelData) {
                    if (error) return reject(error);

                    const objectFormation = {
                      bookedBedCount: hostelData[0]
                        ? hostelData[0].bookedBedCount
                        : 0,
                    ...
                    const bed_query = `SELECT bd.id, bd.bed_no, bd.bed_amount, bd.isfilled,bd.isbooked,isNoticePeriod  
                                       FROM hostelrooms AS hr 
                                       JOIN bed_details AS bd ON bd.hos_detail_id = hr.id 
                                       WHERE hr.Hostel_Id = ? AND hr.Floor_Id = ? AND hr.id = ? 
                                       AND bd.status = 1 AND hr.isActive = 1`;

                    connection.query(
                      bed_query,
                      [
                        objectFormation.Hostel_Id,
                        objectFormation.Floor_Id,
                        objectFormation.Room_Id,
                      ],
                      function (err, bed_data) {
                        objectFormation["bed_details"] = err
                          ? 0
                          : bed_data.length > 0
                          ? bed_data
                          : 0;
                        responseData[i] = objectFormation; // Maintain order
                        resolve();
                      }
                    );
                  }
      ...
      connection.query(query, function (error, hostel_data) {
        if (error) {
          ...
          if (hostel_data && hostel_data.length > 0) {
            ...
            response
              .status(200)
              .json({ hostel_data: Array.from(floorsMap.values()) });
          } else {
    ...
    if (
      ...
      connection.query(sql1, function (err, data) {
        if (err) {
          return response
            .status(201)
            .json({ statusCode: 201, message: "Unable to Get Room Details" });
          ...
          var sql3 =
            "SELECT * FROM hostelrooms WHERE Room_Id=? AND id !=? AND Hostel_Id=? AND isActive=1";
          connection.query(
            sql3,
            [room_id, reqsData.id, hostel_id],
          ...
          return response
            .status(201)
            .json({ statusCode: 201, message: "Invalid Room Details" });
        }
      });
    } else {
      response.status(208).json({
...
function CreateFloor(req, res) {
  ...
  if (
    ...
    var sq1 =
      "SELECT * FROM Hostel_Floor WHERE hostel_id =? AND REPLACE(LOWER(floor_name), ' ', '') = ? AND status=1";
    connection.query(
      sq1,
      [hostel_id, normalizedFloorName],
      function (err, floor_data) {
        if (err) {
          return res
            .status(201)
            .json({ statusCode: 201, message: "Unable to Get Floor Details" });
        } else if (floor_data.length == 0) {
          var sql3 =
            "SELECT * FROM Hostel_Floor WHERE hostel_id='" +
            hostel_id +
          ...
          connection.query(sql3, function (err, fl_data) {
            if (err) {
              return res.status(201).json({
                statusCode: 201,
                message: "Unable to Get Floor Details",
              });
            } else {
              var floor_id;
              if (fl_data.length != 0) {
              ...
              var sql2 =
                "INSERT INTO Hostel_Floor(hostel_id,floor_name,floor_id,status) VALUES (?,?,?,1)";
              connection.query(
                sql2,
                [hostel_id, floor_name, floor_id],
                function (err, ins_data) {
                  if (err) {
                    return res.status(201).json({
                      statusCode: 201,
                      message: "Unable to Add Floor Details",
                    });
                  } else {
                    return res.status(200).json({
                      statusCode: 200,
                      message: "Successfully Added " + floor_name,
                    });
                  }
                }
              );
          ...
          return res
            .status(202)
            .json({ statusCode: 202, message: "Floor Name is Already Exist" });
        }
      }
    );
  } else {
...
function update_floor(req, res) {
  ...
  if (
    ...
    var sq1 =
      "SELECT * FROM Hostel_Floor WHERE hostel_id =? AND REPLACE(LOWER(floor_name), ' ', '') = ? AND status=1 AND floor_id !='" +
      id +
      "'";
    connection.query(
      sq1,
      ...
      function (err, floor_data) {
        if (err) {
          ...
          var sql2 =
            "UPDATE Hostel_Floor SET floor_name=? WHERE hostel_id=? AND floor_id=?";
          connection.query(
            sql2,
            [floor_name, hostel_id, id],
            function (err, up_data) {
              if (err) {
                return res.status(201).json({
                  statusCode: 201,
                ...
                return res.status(200).json({
                  statusCode: 200,
                  message: "Successfully Updated " + floor_name,
                });
              }
            }
          );
...
function UpdateEB(connection, atten, response) {
  if (atten) {
    connection.query(
      `UPDATE hosteldetails SET isHostelBased= ${atten.Ishostelbased} WHERE id='${atten.Id}'`,
      function (error, data) {
        if (error) {
...
function listDashBoard(connection, response, request) {
  ...
  if (
    is_admin == 1 ||
    (role_permissions[0] && role_permissions[0].per_view == 1)
  ) {
    var hostel_id = request.body.hostel_id;
      ...
      if (error) {
        ...
        connection.query(
          ...
          function (err, countdata) {
            if (err) {
              return response.status(201).json({
                message: "Error to Get Count Details",
                error: err.message,
              });
            }
              ...
              if (err) {
                ...
                if (data.length > 0) {
                  ...
                  let dashboardList = data.map((item) => {
                    ...
                    if (pro_res.length != 0) {
                      const projectedAmounts = pro_res.map(
                        (item) => Number(item.projected_amount) || 0
                      );
                      project_amount = projectedAmounts.reduce(
                    ...
                    obj = {
                      ...
                      active_complaint: count_data.activecomp_count,
                      pending_invoice: count_data.pendinginv_count,
                      new_booking: count_data.booking_count,
                      advance_inhand: Number(count_data.advance_inhand),
                    };
                    return obj;
                  });
                  ...
                  // Booking Data
                  var sql4 =
                    "SELECT id,first_name,last_name,amount,joining_date FROM bookings WHERE status=1 AND hostel_id=? ORDER BY id DESC LIMIT 5;";
                  connection.query(
                    sql4,
                    [hostel_id],
                    function (err, book_data) {
                      if (err) {
                        response
                          .status(201)
                          .json({ message: "Error fetching Booking Data" });
                      } else {
                        var sql5 =
                          "SELECT inv.id,inv.Invoices,inv.DueDate,inv.Amount AS total_amount FROM invoicedetails AS inv JOIN hosteldetails AS hs ON hs.id=inv.Hostel_Id WHERE hs.id =? AND inv.invoice_status=1 and inv.BalanceDue !=0 ORDER BY id DESC LIMIT 5;";
                        connection.query(
                          sql5,
                          [hostel_id],
                          function (err, bill_details) {
                            if (err) {
                              response
                                .status(201)
                                .json({ message: "Error fetching Bill Data" });
                            } else {
                              var query1 =
                                "SELECT m.month, COALESCE(SUM(COALESCE(invo.PaidAmount, 0)), 0) AS revenue, COALESCE(SUM(COALESCE(expen.purchase_amount, 0)), 0) AS expense FROM (SELECT DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL n MONTH), '%Y-%m') AS month FROM (SELECT 0 AS n UNION ALL SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5) AS numbers) AS m LEFT JOIN (SELECT DATE_FORMAT(invo.Date, '%Y-%m') AS month, invo.PaidAmount FROM invoicedetails AS invo JOIN hosteldetails AS hos ON hos.id = invo.Hostel_Id WHERE hos.id = ? AND invo.action != 'advance') AS invo ON m.month = invo.month LEFT JOIN (SELECT DATE_FORMAT(expen.createdate, '%Y-%m') AS month, expen.purchase_amount FROM expenses AS expen WHERE expen.hostel_id = ?) AS expen ON m.month = expen.month GROUP BY m.month ORDER BY m.month;";
                              connection.query(
                                query1,
                                [hostel_id, hostel_id],
                                    ...
                                    connection.query(
                                      ...
                                      function (err, advance_data) {
                                        if (error) {
                                          return response.status(201).json({
                                            message:
                                              "Error fetching Advance Details",
                                          });
                                        } else {
                                          // expense category
                                          var sql2 =
                                            "SELECT SUM(expen.purchase_amount) AS purchase_amount, expen.category_id, category.category_Name FROM expenses expen JOIN Expense_Category_Name category ON category.id = expen.category_id WHERE expen.status = true AND expen.hostel_id=? AND YEAR(expen.purchase_date) = YEAR(CURDATE()) AND MONTH(expen.createdate) = MONTH(CURDATE()) GROUP BY expen.category_id";
                                          connection.query(
                                            sql2,
                                            [hostel_id],
                                            function (error, data) {
...
function deleteHostel(request, response) {
  ...
  if (
    is_admin == 1 ||
    (role_permissions[3] && role_permissions[3].per_delete == 1)
  ) {
    if (req && req.hostel_Id) {
      ...
      connection.query(query, function (floorError, floorData) {
        if (floorError) {
          response
            .status(201)
            .json({ message: "Error While Fetching Hostel room details" });
        } else {
          if (floorData && floorData.length != 0) {
            response.status(201).json({
              message: "This hostel has some users, so first delete the users.",
              FloorStatus: 201,
            });
          } else {
            connection.query(
              `SELECT * FROM hosteldetails WHERE id = ${req.hostel_Id} and isActive = true`,
              function (selErr, selData) {
                if (selErr) {
                  response
                    .status(201)
                    .json({ message: "Error While Fetching Hostel details" });
                } else {
                  if (selData && selData.length > 0) {
                    ...
                    connection.query(query, function (delErr, deldata) {
                      if (delErr) {
                        response
                          .status(201)
                          .json({ message: "doesn't update" });
                      } else {
                        connection.query(
                          `select * from Hostel_Floor where hostel_id= ${req.hostel_Id}`,
                          function (err, floorData) {
                            if (err) {
                              return response.status(201).json({
                                message: "Unable to Get Floor Details",
                              });
                            ...
                            if (floorData && floorData.length > 0) {
                              connection.query(
                                `UPDATE Hostel_Floor SET status= false WHERE hostel_id= ${req.hostel_Id}`,
                                function (error, data) {
                                  if (error) {
                                    response
                                      .status(201)
                                      ...
                                      function (room_Error, room_Data) {
                                        if (room_Error) {
                                          ...
                                          if (
                                            ...
                                            connection.query(
                                              `UPDATE hostelrooms SET isActive = false WHERE Hostel_Id='${req.hostel_Id}'`,
                                              function (error, data) {
                                                if (error) {
                                                  response.status(201).json({
                                                    ...
                                                    function (
                                                      ...
                                                      if (bed_Err) {
                                                        ...
                                                        if (
                                                          ...
                                                          var sql2 =
                                                            "UPDATE bed_details SET status= false WHERE hos_detail_id='" +
                                                            req.hostel_Id +
                                                            "'";
                                                          connection.query(
                                                            sql2,
                                                            (err, data) => {
                                                              if (err) {
                                                                res
                                                                  ...
                                                                  .json({
                                                                    message:
                                                                      "Unable to Remove Hostel Details",
                                                                    statusCode: 201,
                                                                  });
                                                              } else {
                                                                response
                    ...
                    response
                      .status(201)
                      .json({ message: "No data found in the hostel" });
                  }
                }
              }
            );
      ...
      connection.query(query, function (roomError, roomData) {
        if (roomError) {
          ...
          if (roomData && roomData.length > 0) {
            response.status(201).json({
              message: "This Floor has some users, so first delete the users.",
              RoomStatus: 201,
            });
          } else {
            connection.query(
              `select * from Hostel_Floor where hostel_id= ${reqData.id} and floor_id =${reqData.floor_id}`,
              function (err, floorData) {
                if (floorData && floorData.length > 0) {
                  connection.query(
                    `UPDATE Hostel_Floor SET status= false WHERE hostel_id= ${reqData.id} AND floor_id =${reqData.floor_id}`,
                    function (error, data) {
                      if (error) {
                        response
                          .status(201)
                          ...
                          function (room_Error, room_Data) {
                            if (room_Error) {
                              ...
                              if (room_Data && room_Data.length > 0) {
                                connection.query(
                                  `UPDATE hostelrooms SET isActive = false WHERE Hostel_Id='${reqData.id}' and Floor_Id = ${reqData.floor_id}`,
                                  function (error, data) {
                                    if (error) {
                                      response
                                        .status(201)
    ...
    if (reqData && reqData.hostelId && reqData.floorId && reqData.roomNo) {
      ...
      connection.query(query, function (sel_Error, selData) {
        if (sel_Error) {
          ...
          if (selData.length != 0) {
            response.status(201).json({
              message: "This Room has some users, so first delete the users.",
              BedStatus: 201,
            });
          } else {
            ...
            connection.query(query1, function (room_Error, room_Data) {
              if (room_Error) {
                ...
                response
                  .status(201)
                  .json({ message: "Error while fetching Room details" });
              } else {
                if (room_Data && room_Data.length > 0) {
                  connection.query(
                    `UPDATE hostelrooms SET isActive = false WHERE Hostel_Id='${reqData.hostelId}' AND id= '${reqData.roomNo}' AND Floor_Id='${reqData.floorId}';`,
                    function (error, data) {
                      if (error) {
                        response
                          .status(201)
                        ...
                        response
                          .status(200)
                          .json({ message: "Room Update Successfully" });
                      }
                    }
                  );
                } else {
...
function deleteBed(req, res) {
  ...
  if (
    ...
    var sql1 =
      "SELECT *,bd.id AS bed_detail_id FROM hostelrooms AS hr JOIN bed_details AS bd ON bd.hos_detail_id=hr.id WHERE bd.bed_no='" +
      bed_id +
      "' AND hr.Hostel_Id='" +
      hostelId +
      "' AND hr.Floor_Id='" +
      ...
      "' AND hr.id='" +
      roomNo +
      "' AND bd.status=1 AND hr.isActive= true";
    connection.query(sql1, (err, data) => {
      if (err) {
        ...
        if (isfilled == 0) {
          // Update Status 0
          var sql2 =
            "UPDATE bed_details SET status= false WHERE id='" +
            bed_detail_id +
            "'";
          connection.query(sql2, (err, data) => {
            if (err) {
              res.status(201).json({
                message: "Unable to Remove Hostel Details",
                statusCode: 201,
              });
            } else {
              res
                .status(200)
                .json({ message: "Sucessfully Bed Removed", statusCode: 200 });
            }
          });
        } else {
          res.status(210).json({
            message: "User Assigned In this Bed, So Not Remove this Bed",
            statusCode: 210,
          });
        }
      } else {
...
function get_room_details(connection, request, response) {
  ...
  if (
    ...
    if (
      ...
      response
        .status(201)
        .json({ message: "Missing Parameter Values", statusCode: 201 });
    } else {
      var sql1 =
        "SELECT * FROM hosteldetails WHERE id='" +
        hostel_id +
        "' AND isActive =1;";
      connection.query(sql1, function (sq_err, sq_res) {
        if (sq_err) {
          ...
          var sql2 =
            "SELECT * FROM hostelrooms WHERE Hostel_Id=? AND Floor_Id=? AND id=? AND isActive= true;";
          connection.query(
            sql2,
            [hostel_id, floor_id, room_id],
            function (room_err, room_res) {
              if (room_err) {
                ...
                response.status(200).json({
                  ...
                  statusCode: 200,
                  room_details: room_res[0],
                });
              } else {
                response.status(201).json({
                  message: "Invalid Or Inactive Room ID",
                  statusCode: 201,
                });
              }
            }
          ...
          response.status(201).json({
            message: "Invalid or Inactive Hostel Details",
            statusCode: 201,
          });
        }
      });
  ...
  if (
    ...
    connection.query(sql1, function (sq_err, sq_res) {
      if (sq_err) {
        ...
        var sql2 =
          "SELECT * FROM hostelrooms WHERE Hostel_Id=? AND Floor_Id=? AND id=?";
        connection.query(
          sql2,
          [hostel_id, floor_id, room_id],
          function (room_err, room_res) {
            if (room_err) {
                ...
                function (up_err, up_res) {
                  if (up_err) {
                    ...
                    var update_user_query =
                      "UPDATE hostel SET RoomRent=? WHERE Hostel_Id=? AND Floor=? AND Rooms=? AND isActive=1";
                    connection.query(
                      update_user_query,
                      [amount, hostel_id, floor_id, room_id],
                      function (err, data) {
                        if (err) {
                          response.status(201).json({
                            message: "Unable to Update Users Rent",
                            statusCode: 201,
                          });
                        } else {
                          response.status(200).json({
              ...
              response
                .status(201)
                .json({ message: "Invalid Room ID", statusCode: 201 });
            }
          }
        );
      } else {
...
function createBed(req, res) {
  ...
  if (
    ...
    if (!hostel_id || !floor_id || !room_id || !bed_no || !amount) {
      return res
        .status(201)
        .json({ statusCode: 201, message: "Missing Parameters." });
    }

    if (isNaN(amount)) {
      return res
        .status(201)
        .json({ statusCode: 201, message: "Amount must be a number." });
    }
    ...
    connection.query(sql1, (err, hs_data) => {
      if (err) {
        ...
        var sql2 =
          "SELECT * FROM hostelrooms WHERE Hostel_Id=? AND Floor_Id=? AND id=? AND isActive= true AND Created_By='" +
          created_by +
          "'";
        connection.query(
          sql2,
          ...
          (err, hs_res) => {
            if (err) {
              return res.status(201).json({
                statusCode: 201,
                message: "Unable to Get Hostel Room Details",
              });
              ...
              // Check Bed Details
              var sql4 =
                "SELECT * FROM bed_details WHERE bed_no='" +
                bed_no +
                "' AND status=1 AND hos_detail_id='" +
                hos_detail_id +
                "'";
              connection.query(sql4, (err, bed_res) => {
                if (err) {
                  return res.status(201).json({
                    statusCode: 201,
                    message: "Unable to Get Bed Details",
                  });
                  ...
                  var sql5 =
                    "INSERT INTO bed_details (hos_detail_id,bed_no,bed_amount,status,createdby) VALUES (?,?,?,1,?)";
                  connection.query(
                    sql5,
                    [hos_detail_id, bed_no, amount, created_by],
                    (err, ins_data) => {
                      if (err) {
                        return res.status(201).json({
                          statusCode: 201,
                          message: "Unable to Add Bed Details",
                        });
                      } else {
                        var number_of_beds = total_beds + 1;
                        var sql3 =
                          "UPDATE hostelrooms SET Number_Of_Beds='" +
                          number_of_beds +
                          "' WHERE id='" +
                          hos_detail_id +
                          "'";
                        connection.query(sql3, (up_err, up_res) => {
                          if (up_err) {
                            return res.status(201).json({
                              statusCode: 201,
                              message: "Unable to Update Total Bed Details",
                            });
                          } else {
                            return res.status(200).json({
                              statusCode: 200,
                              message: "Successfully Add New Bed Details",
                            });
                          }
                        });
                      }
        ...
        return res
          .status(201)
          .json({ statusCode: 201, message: "Invalid Hostel ID" });
      }
    });
  } else {
    res.status(208).json({
  ...
  if (
    ...
    connection.query(sql1, (err, hs_data) => {
      if (err) {
        ...
        var sql2 =
          "SELECT hs.id as Room_Id,hs.Hostel_Id,hs.Floor_Id,hs.Room_Id as Room_Name,hs.Number_Of_Beds,hs.isActive as Room_Status,hs.Created_By,hf.id,hf.floor_id,hf.floor_name,hf.status as Floor_Status,hs.id AS bed_details_id FROM hostelrooms AS hs JOIN Hostel_Floor AS hf ON hf.hostel_id=hs.Hostel_Id AND hf.floor_id=hs.Floor_Id WHERE hs.Hostel_Id=? AND hs.Floor_Id=? AND hs.id=? AND hs.isActive=1 AND hs.Created_By IN (?)";
        connection.query(
          sql2,
          [hostel_id, floor_id, room_id, show_ids],
          (err, hs_res) => {
            if (err) {
              return res.status(201).json({
                statusCode: 201,
              ...
              return res.status(201).json({
                statusCode: 201,
                message: "Invalid Hostel or Room Details",
              });
            }
          }
        );
...
function delete_hostel_image(req, res) {
  ...
  if (
    ...
    var sql1 =
      "SELECT " +
      image_name +
      " AS image FROM hosteldetails WHERE id=? AND isActive=1 AND created_By=?";
    connection.query(sql1, [hostel_id, created_by], async function (err, data) {
      if (err) {
        return res
          .status(201)
          .json({ statusCode: 201, message: "Unable to Get PG Details" });
      } else if (data.length != 0) {
        ...
        connection.query(sql2, function (err, up_data) {
          if (err) {
            return res
              .status(201)
              .json({ statusCode: 201, message: "Unable to Delete Image" });
          } else {
            return res.status(200).json({
              statusCode: 200,
        ...
        return res
          .status(201)
          .json({ statusCode: 201, message: "Invalid PG Details" });
      }
    });
  } else {
    res.status(208).json({
...
function hosteldetails(req, res) {
  ...
  if (user_type == "staff") {
    var sql2 =
      "SELECT ro.hostel_id FROM createaccount AS ca JOIN roles AS ro ON ca.role_id=ro.id WHERE ca.id=? AND ca.user_status=1 AND ro.status=1 ORDER BY ca.id DESC;";
    connection.query(sql2, [created_by], function (err, data) {
      if (err) {
        ...
        var sql1 =
          "SELECT id,Name,profile,email_id,hostel_PhoneNo AS phone,Address,area,landmark,pin_code,city,state FROM hosteldetails WHERE isActive=1 AND id=?";
