                        ...
                        return new Promise((resolve, reject) => {
                            ...
                            connection.query(bookedBedCountQuery, [reqFloorID.floor_Id, reqFloorID.hostel_Id, Room_Id], function (error, hostelData) {
                                ...
                                connection.query(bed_query, [objectFormation.Hostel_Id, objectFormation.Floor_Id, objectFormation.Room_Id], function (err, bed_data) {
                                    objectFormation['bed_details'] = err ? 0 : (bed_data.length > 0 ? bed_data : 0);
    ...
    if (is_admin == 1 || (role_permissions[0] && role_permissions[0].per_view == 1)) {
        ...
        connection.query(sql1, [hostel_id], function (error, data) {
            if (error) {
                ...
                connection.query(sql11, [hostel_id, hostel_id, hostel_id, hostel_id], function (err, countdata) {
                    if (err) {
                    ...
                    connection.query(sql12, [hostel_id], function (err, pro_res) {
                        if (err) {
                            ...
                            if (data.length > 0) {
                                ...
                                response.status(200).json({ dashboardList: [], Revenue_reports: [], totalAmount: [], categoryList: [], advance_data: [] });
                            }
                        }
...
function hosteldetails(req, res) {
    ...
    if (user_type == 'staff') {
        ...
        var sql1 = "SELECT id,Name,image1,image2,image3,image4,email_id,hostel_PhoneNo AS phone,Address,area,landmark,pin_code,city,state ,profile FROM hosteldetails WHERE isActive=1 AND created_By IN (?)";
