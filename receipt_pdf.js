    ...
    connection.query(sql1, receipt_id, function (err, data) {
        if (err) {
            ...
            if (invoice_number == 0) {
                ...
                var sql2 = "SELECT id,action,hos_user_id FROM invoicedetails WHERE Invoices=? AND Hostel_Id=?";
                connection.query(sql2, [invoice_number, hostel_id], function (err, inv_details) {
                    if (err) {
                        ...
                        var sql2 = "SELECT * FROM manual_invoice_amenities WHERE invoice_id=? AND user_id=?";
                        connection.query(sql2, [invoice_id, user_id], function (err, amenities) {
                            if (err) {
