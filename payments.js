...
function add_bank(req, res) {
    ...
    if (!benificiary_name) {
        return res.status(201).json({ statusCode: 201, message: "Missing Benificiary Details" })
    }
    ...
    if (is_admin == 1 || (role_permissions[16] && role_permissions[16].per_view == 1)) {
        ...
        connection.query(sql1, [hostel_id], function (err, data) {
            if (err) {
                ...
                var sql2 = "SELECT trans.*,ban.benificiary_name,ban.type FROM bank_transactions AS trans JOIN bankings AS ban ON ban.id=trans.bank_id WHERE trans.status=1 AND ban.hostel_id=? ORDER BY trans.id DESC";
