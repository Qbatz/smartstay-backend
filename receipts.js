...
exports.get_all_receipts = (req, res) => {
    ...
    if (is_admin == 1 || (role_permissions[10] && role_permissions[10].per_edit == 1)) {
        ...
        var sql1 = `SELECT re.*, hos.Name AS Name, hos.profile AS user_profile,hos.return_advance, inv.id AS inv_id, re.id AS id,hos.Address AS user_address, ca.Address AS admin_address FROM receipts AS re JOIN hostel AS hos ON hos.id = re.user_id LEFT JOIN (SELECT * FROM invoicedetails WHERE Hostel_Id = ? AND invoice_status = 1GROUP BY Invoices) AS inv ON inv.Invoices = re.invoice_numberJOIN createaccount AS ca ON ca.id = hos.created_by WHERE hos.Hostel_Id = ? AND re.status = 1ORDER BY re.id DESC;`;
